...
If your `logima-pwa-api` instance is running on the `localhost`, port `8080` then the correct elasticsearch endpoint is as presented here.

Starting from Vue Storefront v1.6 user may set the: `config.elasticsearch.queryMethod` to either "POST" (default) / "GET". When "GET" is set, the ElasticSearch Query object is passed to logima-pwa-api as a request parameter named "request". By doing so Service Worker will now able to cache the results from ElasticSearch. Service Workers cannot cache any POST requests currently.

:::tip Notice
Service worker is not caching the /api requests on development envs. (localhost) as the logima-pwa-api by default runs on a different port (8080)
:::
...
This option is referenced in the [core/client-entry.ts](https://github.com/LogimaeCommerce/logima-pwa/blob/master/core/client-entry.ts) line: 85.
...
You should add all the multistore codes to the `mapStoreUrlsFor` as this property is used by [core/lib/multistore.ts](https://github.com/LogimaeCommerce/logima-pwa/blob/master/core/lib/multistore.ts) -> `setupMultistoreRoutes` method to add the `/<store_code>/p/....` and other standard routes. By accessing them you're [instructing Vue Storefront to switch the current store](https://github.com/LogimaeCommerce/logima-pwa/blob/master/core/client-entry.ts) settings (i18n, API requests with specific storeCode etc...)
...
This URL is used only in the `Switcher` component. Typically it equals just to `/<store_code>`. Sometimes you may like to have different store views running as separate Vue Storefront instances; even under different URL addresses. This is the situation when this property comes into action. Just take a look at how [Language/Switcher.vue](https://github.com/LogimaeCommerce/logima-pwa/blob/master/src/themes/default/components/core/blocks/Switcher/Language.vue) generates the list of the stores.
...
The internationalization settings are used by the translation engine (`defautlLocale`) and the [Language/Switcher.vue](https://github.com/LogimaeCommerce/logima-pwa/blob/master/src/themes/default/components/core/blocks/Switcher/Language.vue) (`fullCountryName`, `fullLanguageName`). `currencyCode` is used for some of the API calls (rendering prices mostly) and `currencySign` is being used for displaying the prices in the frontend.
...
This option property is referenced in the [core/modules/catalog/store/product](https://github.com/LogimaeCommerce/logima-pwa/tree/master/core/modules/catalog/store/product), [core/modules/catalog/store/category](https://github.com/LogimaeCommerce/logima-pwa/tree/master/core/modules/catalog/store/category), [core/modules/catalog/store/attribute](https://github.com/LogimaeCommerce/logima-pwa/tree/master/core/modules/catalog/store/attribute)

```json
  "twoStageCaching": true,
```

Vue Storefront caches all the data entities got from `logima-pwa-api` into indexedDB local cache. This is a key feature for providing users with offline mode. Unfortunately, when the `entities.optimize` option is set to `true`, we cannot cache the optimized entities as they don't contain all the required information.
...
Please take a look at the [core/modules/catalog/store/category](https://github.com/LogimaeCommerce/logima-pwa/tree/master/core/modules/catalog/store/category) for the reference.
...
Please take a look at the [core/modules/cart](https://github.com/LogimaeCommerce/logima-pwa/tree/master/core/modules/cart).
...
Please have a look at the [core/modules/cart](https://github.com/LogimaeCommerce/logima-pwa/tree/master/core/modules/cart) for a reference.
...
Please take a look at the [core/modules/cart](https://github.com/LogimaeCommerce/logima-pwa/tree/master/core/modules/cart) for a reference.
...
These endpoints should point to the `logima-pwa-api` instance and typically you're changing just the domain-name/base-url without touching the specific endpoint URLs as it's related to the `logima-pwa-api` specifics.
...
Please take a look at the [core/lib/search.ts](https://github.com/LogimaeCommerce/logima-pwa/tree/master/core/lib/search.ts) and [core/modules/catalog/store/category/mutations.ts](https://github.com/LogimaeCommerce/logima-pwa/tree/master/core/modules/catalog/store/category/mutations.ts) for reference

**Please note:** As `url_key` field must be unique across categories collection. Therefore - we're by default generating its value based on name + category id. Please [switch this option off](https://github.com/LogimaeCommerce/mage2vuestorefront/#initial-logima-pwa-import) if You'd like to keep the `url_key` as they come from Magento2.
...
This property is used in the [core/store/modules/product/actions.ts](https://github.com/LogimaeCommerce/logima-pwa/tree/master/core/store/modules/product/actions.ts); if it's set to `true` Vue Storefront will query the `logima-pwa-api` endpoint (`/api/products/render-list`) to render the product prices for currently displayed product(s) **every time** user is about to display the product or category page.
...
This is the `logima-pwa-api` endpoint for rendering product lists.

```json
  "defaultFilters": ["color", "size", "price", "erin_recommends"],
```

`defaultFilters` array should contain **all** the filters that could be used in the [Sidebar menu filters](https://github.com/LogimaeCommerce/logima-pwa/tree/master/src/themes/default/components/core/blocks/Category/Sidebar.vue).
...
This is a simple map used in the [core/pages/Checkout.js](https://github.com/LogimaeCommerce/logima-pwa/tree/master/core/pages/Checkout.js) to map the payment methods provided by the backend service with the ones available to the Vue Storefront. Each payment method is a separate Vue Storefront extension and not all methods provided by the backend should necessarily be supported by the frontend.
...
Please check the [core/service-worker/order.js](https://github.com/LogimaeCommerce/logima-pwa/tree/master/core/service-worker/order.js) for reference
...
When the `autoRefreshTokens` property is set to `true` (default) Vue Storefront will be trying to refresh the user tokens automatically when the session ends up. Please take a look at the [core/lib/sync/task.ts](https://github.com/LogimaeCommerce/logima-pwa/tree/master/core/lib/sync/task.ts) for reference.
...
This section is to set the default base URL of product images. This should be a `logima-pwa-api` URL - pointing to it's `/api/img` handler. Vue Storefront API is in charge of downloading the local image cache from the Magento/Pimcore backend and does the resize/crop/scale operations to optimize the images for mobile devices and the UI.
...
"install": {
  ...
  "backend_dir": "../logima-pwa-api"
},
```

This is just to be used in the [core/scripts/installer.js](https://github.com/LogimaeCommerce/logima-pwa/tree/master/core/scripts/installer.js)
...
Internationalization settings are used by the translation engine (`defautlLocale`) and the [Language/Switcher.vue](https://github.com/LogimaeCommerce/logima-pwa/tree/master/src/themes/default/components/core/blocks/Switcher/Language.vue) (`fullCountryName`, `fullLanguageName`). `currencyCode` is used for some of the API calls (rendering prices mostly) and `currencySign` is being used for displaying the prices in the frontend.
...
This property is used by the mailchimp extension (See [src/extensions](https://github.com/LogimaeCommerce/logima-pwa-api/tree/master/src/api/extensions) for a reference).

## Theme

```json
"theme": "@logima-pwa/theme-default",
```
...
This is the URL endpoint of the Snow.dog Magento2 CMS extensions - need to be set when using the [src/api/extensions/cms-data](https://github.com/LogimaeCommerce/logima-pwa-api/tree/master/src/api/extensions/cms-data)
