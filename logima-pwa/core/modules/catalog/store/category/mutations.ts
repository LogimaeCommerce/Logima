...
import { slugify, breadCrumbRoutes } from '@logima-pwa/core/helpers'
    ...
    for (let category of categories.items) {
      let catSlugSetter = (category) => {
        if (category.children_data) {
          for (let subcat of category.children_data) { // TODO: fixme and move slug setting to logima-pwa-api
            if (subcat.name) {
