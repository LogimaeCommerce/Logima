...
const actions: ActionTree<PromotedOffersState, RootState> = {
  async updatePromotedOffers ({commit, rootState}, data) {
    ...
    try {
      const promotedOffersModule = await import(/* webpackChunkName: "logima-promoted-offers-[request]" */ `theme/resource/${promotedBannersResource}.json`)
  ...
  async updateHeadImage ({commit, rootState}, data) {
    ...
    try {
      const imageModule = await import(/* webpackChunkName: "logima-head-img-[request]" */ `theme/resource/${mainImageResource}.json`)
