...
## Problem starting Docker while installing the logima-pwa
...
│                                                                            │
│ Please check log file for details: /tmp/logima-pwa/var/log/install.log │
└────────────────────────────────────────────────────────────────────────────┘
```
...
- please check the output of running `docker-compose up -d` manually inside the `logima-pwa-api` instance. On some production environments docker is limited for the superusers, in many cases it's just a matter of `/var/run/docker.sock` permissions to be changed (for example to 755)
...
See the discussion in [#137](https://github.com/LogimaeCommerce/logima-pwa/issues/137).
Please also check the [Database tool](https://github.com/LogimaeCommerce/logima-pwa/blob/master/doc/Database%20tool.md)
...
- modify the [core](https://github.com/LogimaeCommerce/logima-pwa/blob/6a5a569a7e96703b865f841dabbe3c6a1020b3ab/core/store/modules/product/actions.js#L311) - to filter out the `name` attribute from `Object.assign` which is responsible for copying the attributes from variant -> current product,
- modify `mage2vuestorefront` importer to correct the `configurable_children` [product names](https://github.com/LogimaeCommerce/mage2vuestorefront/blob/ca0c4723530b148cfdfb99784168af529e39d599/src/adapters/magento/product.js#L167)
...
After following the Tutorial on [how to connect to Magento2](../installation/magento.md) the prices are updated just after manually running [mage2vuestorefront cli command](https://github.com/LogimaeCommerce/mage2vuestorefront).
...
To make it work you need have Magento2 Oauth keys configured in your `logima-pwa-api` - `conf/local.json`.
...
Take a look at the discussion at [#644](https://github.com/LogimaeCommerce/logima-pwa/issues/644)
...
```bash
git clone https://github.com/LogimaeCommerce/logima-pwa.git logima-pwa && cd logima-pwa
npm install
...
Yes, I believe it could. You should expose the API accordingly to our [spec](../extensions/extending-api.md) and the second step is to [create a data bridge](https://medium.com/@piotrkarwatka/how-to-connect-3rd-party-platform-to-logima-pwa-df9cb30779f6) to fill out the ElasticSearch with the current catalog data.

## Is there any documentation on integrating payment gateways?

We're working on kind of boilerplate for payment modules. Right now please just take a look at a [live example](https://github.com/develodesign/logima-pwa-stripe) and try to follow the design patterns from there. The task where boilerplate + docs will show up is [https://github.com/LogimaeCommerce/logima-pwa/issues/923](https://github.com/LogimaeCommerce/logima-pwa/issues/923).
...
If you would like to have a Category filter working with configurable products, you need to expand the `product.configurable_children.attrName` to `product.attrName_options` array. This is automatically done by [mage2vuestorefront](https://github.com/LogimaeCommerce/mage2vuestorefront) for all attributes set as `product.configurable_options` (by default: color, size). If you like to add additional fields like `manufacturer` to the filters, you need to expand `product.manufacturer_options` field. The easiest way to do so is to set `config.product.expandConfigurableFilters` to `['manufacturer']` and re-run the `mage2vuestorefront` indexer.

## How to redirect original Magento2 URLs to Vue Storefront

There is an SEO redirects generator for NGINX -> `https://serverfault.com/a/441517` available within the [logima-pwa-api](https://github.com/LogimaeCommerce/logima-pwa-api/commit/2c7e10b4c4294f222f7a1aae96627d6a0e23f30e). Now you can generate SEO map redirecting users from the original Magento URLs to Vue Storefront URLs by running:

```bash
npm run seo redirects — —oldFormat=true | false
```

- `oldFormat` - should be set accordingly to the `logima-pwa/config/local.json` setting of `products.useShortCatalogUrls` (`oldFormat` = `!useShortCatalogUrls`)

Please make sure that `logima-pwa/config/local.json` setting of `useMagentoUrlKeys` is set to `true` and you have ElasticSearch synchronized with the Magento2 instance using the current version of [mage2vuestorefront](https://github.com/LogimaeCommerce/mage2vuestorefront).
...
This is because the demo data dump works on the `demo-magento2.vuestorefront.io` instance attribute ids. Please re-import all product data using [mage2vuestorefront](https://github.com/LogimaeCommerce/mage2vuestorefront)
...
And then You can use proper controls for each individual filter [here](https://github.com/LogimaeCommerce/logima-pwa/blob/49dc8a2dc9326e9e83d663cc27f8bb0688525f13/src/themes/default/components/core/blocks/Category/Sidebar.vue).
...
## Running logima-pwa-api on a different machine than magento / images not working
When you separate logima-pwa-api and magento2 by putting them on different servers, it is necessary to link the logima-pwa-api machine with magento media folder via network folder. `sshsfs` is suggested for this.
Once the network connection is established, the correct folder needs to be pointed in the logima-pwa-api config
```
"assetPath": "/../var/magento2-sample-data/pub/media", 
```
It is necessary for the correct product image creation by the logima-pwa-api. If this is not set corectly, images will just be placeholders regardless the fact the rest of the setup works and you can see products and categories correctly synced.

## Syncing magento and logima-pwa-api servers / Fix for "An error occurred validating the nonce"
