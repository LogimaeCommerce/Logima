...
import { getThumbnailPath } from '@logima-pwa/core/helpers'
          ...
          if (confChildSkus.length > 0) {
            context.dispatch('stock/list', { skus: confChildSkus }, {root: true}).then((task) => {
              if (task && task.resultCode === 200) {
                ...
                Logger.error('Cannot sync the availability of the product options. Please update the logima-pwa-api or switch on the Internet', 'helper')()
              }
            }).catch(err => {
