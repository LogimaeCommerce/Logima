...
- Docker (with [docker-compose](https://docs.docker.com/compose/install/) installed).
  ...
  Already included in `logima-pwa` and `logima-pwa-api` Docker images (required locally, if you do not use containerization):
...
If you answer `Yes`, you will have remote backend at `https://demo.vuestorefront.io`. Otherwise, you will need to install `logima-pwa-api`.
...
You can simply proceed and as a result you will have a `logima-pwa` folder inside your project root and Storefront application running on `http://localhost:3000`. All images will be also hosted at `https://demo.vuestorefront.io/img/`.

### Installing the logima-pwa-api locally
...
? Please provide path for installing backend locally ../logima-pwa-api
...
│ Congratulations!                                               │
│                                                                │
│ You've just successfully installed logima-pwa.             │
│ All required servers are running in background                 │
│                                                                │
│ Storefront: http://localhost:3000                              │
│ Backend: http://localhost:8080                                 │
│                                                                │
│ Logs: /Users/natalia/Desktop/work/test/logima-pwa/var/log/ │
│                                                                │
│ Good Luck!                                                     │
└────────────────────────────────────────────────────────────────┘
```

Your project should contain 2 folders at this moment: `logima-pwa` and `logima-pwa-api`. Vue Storefront should be running on `http://localhost:3000`:

![Storefront screenshot](../images/storefront.png)

## Manual installation

### Install the logima-pwa-api

You need to use [https://github.com/LogimaeCommerce/logima-pwa-api](https://github.com/LogimaeCommerce/logima-pwa-api). It's the ultimate API backend for this application.
...
To test out the application you'll need some test data. In `logima-pwa-api/var/catalog.json` you have data dump for ElasticSearch with default Magento2 products database. We're using for development purposes.
...
The config file is quite simple, but here you have some comments: [Config file for logima-pwa](https://github.com/LogimaeCommerce/logima-pwa/wiki/Config-file-format-for-logima-pwa).
...
Clone the image files for default product database (we're using [Magento2 example products dataset](https://github.com/magento/magento2-sample-data). Please execute the following command in **the root folder of logima-pwa-api project**:
...
### Install the logima-pwa

First step is to clone [logima-pwa](https://github.com/LogimaeCommerce/logima-pwa)
...
Finally, you have to choose between two modes of running the application (similarly as in the case of logima-pwa-api).
