...
Vue Storefront is supporting PayPal payments with PayPal Payment extension for [logima-pwa](https://github.com/LogimaeCommerce/logima-pwa), by [Develo Design](https://www.develodesign.co.uk).
...
By hand (preferer):
```shell
$ git clone git@github.com:develodesign/logima-payment-paypal.git ./logima-pwa/src/modules/paypal
```

Or as dependency. Add the extension to your Vue Storefront `package.json` using:
```shell
$ npm install logima-payment-paypal --save
```
...
Install additional extension for `logima-pwa-api`:
```shell
$ mkdir -p ../logima-pwa-api/src/api/extensions/payment-paypal
$ cp -f ./api-ext/index.js ../logima-pwa-api/src/api/extensions/payment-paypal/
```

Go to api config  `./logima-pwa-api/config/local.json` and register the Paypal Api extension:
```
